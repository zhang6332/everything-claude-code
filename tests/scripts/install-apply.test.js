...
function runTests() {
  ...
  if (test('installs Cursor configs and writes install-state', () => {
    ...
    try {
      ...
      assert.ok(mcpConfig.mcpServers.exa, 'Should install the bundled exa MCP server into Cursor');
  ...
  if (test('installs Cursor MCP config by merging bundled servers into an existing mcp.json', () => {
    ...
    try {
      ...
      assert.ok(mcpConfig.mcpServers.exa, 'Should merge bundled exa MCP server');
    } finally {
      cleanup(homeDir);
      cleanup(projectDir);
    }
